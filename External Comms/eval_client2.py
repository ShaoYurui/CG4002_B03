...
class eval_client2(threading.Thread):
        ...
        while True:
            if self.game_mode != 2:
                ...
                if updated_gamestate != "no_update":
                    ...
                    self.predicted_gamestate = {"p1": self.p1.get_dict(), "p2": self.p2.get_dict()}
                    self.eval_gamestate_queue.put(self.predicted_gamestate)
                    #print("Data put into eval queue")
                    try: 
                        self.prediction_value = self.prediction_queue.get_nowait()
                        self.handle_gamestate()
                        #self.calc_gamestate_queue.put(self.predicted_gamestate)
                        #print("Data put into calc queue")
                        self.send_data()
                    except Empty:
